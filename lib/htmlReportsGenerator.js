...
class HTMLReportsGenerator {
	...
	generateReport() {
		if (!this.options.errorFileFunction) {
			...
			filePathTemp = filePathTemp.slice(filePathTemp.length-2).slice(1,2).join("-").replace(/\.[^/.]+$/, "").replace(/[,<>=?|*:."%]/g, '');

			this.filePath = filePathTemp + ".html";
		}
		else if (typeof this.options.errorFileFunction === 'function') {
			this.filePath = this.options.errorFileFunction( this.report['filename'] );
		}

		this.folderPath = this.options.useTimeStamp ? this.options.reportFolderName+"-" + dateObj.getDate + '-' + dateObj.getSimplifyTime : this.options.reportFolderName;
		...
		if (this.options.verbose) gutil.log('HTML Validation report generated: '.info + errorCompletePath);
	}
}
